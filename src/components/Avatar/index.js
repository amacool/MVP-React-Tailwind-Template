...
const Avatar = ({ altName, imageURL, avatarLabel, bgColor, size }) => {
    ...
    return (
        <div 
        className={imageURL ? `m-2 w-${avatarSize} h-${avatarSize} rounded-full overflow-hidden` 
        : 
        `w-${avatarSize} h-${avatarSize} text-white m-2 flex justify-center items-center text-xl ${bgColor} rounded-full`}>
